...
# -*- coding: utf-8 -*-
"""
LLM disease-name prediction: CANDIDATE-CONSTRAINED SELECTION task.

The model must first choose up to three ICD-10 categories and then, within
each chosen category, select up to three disease names from the fixed set of
55 candidate labels (Supplementary Table 8). The candidate set was defined
from the hospital respiratory disease-name system before the experiments and
was not adjusted to the label distribution of the evaluation records.
...
This file contains the ICD-guided prompt. The direct-prompt condition used
the same script with the ICD selection step removed, keeping the identical
55-label candidate set and the identical output requirements.
...
Usage:
  set OPENAI_API_KEY=...
  python run_constrained_choice.py --input records.xlsx ^
      --model gpt-4.1 --output GPT4.1_constrained_choice.xlsx
"""
...
# ========= prompt construction: up to 3 ICD categories, then up to 3 labels under each =========
